...
export default function SiteSettingsPage() {
  ...
  // Pre-populate form when site data loads
  useEffect(() => {
    if (site && !initialized) {
      setName(site.name ?? "");
      setUrl(site.url ?? "");
      setCreds({
        cpanelHost: site.cpanelHost ?? "",
        cpanelPort: String(site.cpanelPort ?? "2083"),
        cpanelUsername: site.cpanelUsername ?? "",
        cpanelToken: site.cpanelToken ?? "",
        wpRestUrl: site.wpRestUrl ?? "",
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Update credentials and access layers for {site.name}
          </p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            ...
            <p className="text-xs text-muted-foreground">
              Add or update your credentials
            </p>
          </div>
        ...
        <div className="flex justify-end gap-3">
          <Button type="button" variant="outline" asChild>
            <Link href={`/dashboard/site/${siteId}`}>Cancel</Link>
          </Button>
